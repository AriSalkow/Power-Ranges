...
1) weather data
2) model for meter
3) JSON of scaling parameters
4) csv of public holiday dates
"""
...
def predict_fn(meter_id, weather_data_df):
    ...
    del model
    return timestamps, preds #pd.DataFrame(preds, index=df.index[TIMESTEPS - 1:], columns=['consumption_h'])


if __name__ == '__main__':
    meter_id = 200713
    weather_data_df = pd.read_csv(str(meter_id) + '_weather_feats.csv', index_col='captured_on_h', parse_dates=True)
    predictions_df = predict_fn(meter_id, weather_data_df)
    print(predictions_df.head())
    # JSON with 2 keys
    # API gives 1 param, 'meter_id'
    #
